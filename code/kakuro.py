...
def parseInputs(inputfile):      #reads input file to get num of rows, cols, given matrices
   ...
   return inpHoris,inpVerts,nRows,nCols

def getConstraintMatrix(inpHoris,inpVerts,nRows,nCols):     # generates binary constraint matrix and also returns domain of each type of variables
   ConstraintMatrix=[]
   Domain=dict()
   DomainUi=[]
   # consNum=0
      ...
      for j in range(nCols):
         if inpHoris[i][j]=='#':
            ...
            if j==nCols-1:
               ...
               #
               zeroIndxs=[]
   # print("getConstraintMatrix ends")
   ...
   return ConstraintMatrix,Domain,DomainUi
...
def AC3(allArcs,ConstraintMatrix,Domain,DomainUi,nCols):   # function AC-3(csp) returns false if an inconsistency is found and true otherwise
   #  print("inside AC3")
    q = Queue() 
        ...
        if typ=='x':
            ...
            for cmv in range(len(ConstraintMatrix[Xj][:])):
                ...
                if ConstraintMatrix[Xj][cmv]==1:
                    posInDj+=1
            # print("preCall Di=",Domain[(Xi_x,Xi_y)])
...
def mac(unassignedVar,assignment,rowNeighbours,colNeighbours,Domain,inpHoris,inpVerts):      # maintain arc consistency
   arc=[]
   macreturn=True
   for nrs in rowNeighbours:
      if assignment[nrs]!=-1:
         arc.append((nrs,unassignedVar))
   for nrs in colNeighbours:
      if assignment[nrs]!=-1:
         arc.append((nrs,unassignedVar))
   #print(",")
   macreturn=AC3_Mac(arc,Domain,inpHoris,inpVerts,assignment)
   #print(",,")
   return macreturn,Domain

def backTrack(assignment,Domain,inpHoris,inpVerts,nRows,nCols,isMac):      #function that implements backtracking search
   unassignedVar=(-1,-1)
   for p in assignment:
      if assignment[p]==-1:
         unassignedVar=p
   ...
   for val in dom_una_cpy:
      ...
      if numConf==0:
         ...
         if not isMac:
            result=backTrack(assignment,Domain,inpHoris,inpVerts,nRows,nCols,isMac)
            if result is not None:
               return result
         else:
            # print("..")
            macreturn,Domainret=mac(unassignedVar,assignment,rowNeighbours,colNeighbours,Domain,inpHoris,inpVerts)
            # print("...")
            if macreturn:
               # Domain=Domainret
               result=backTrack(assignment,Domainret,inpHoris,inpVerts,nRows,nCols,isMac)
               if result is not None:
                  return result
...
def main(argv):
   ...
   try:
      inpHoris,inpVerts,nRows,nCols=parseInputs(inputfile)              # convert input file format to array format for easy access
   except FileNotFoundError:
      print("Error: Invalid input file\n")
      sys.exit(1)

   ConstraintMatrix,Domain,DomainUi=getConstraintMatrix(inpHoris,inpVerts,nRows,nCols)    # generate constraint matrix, Domain of X variables, Domain of U variables 
   ...
   # print("AC-3 begins")
   worked,Domain,DomainUi=AC3(allArcs,ConstraintMatrix,Domain,DomainUi,nCols)          # Apply arc consistency
   # print("AC-3 ends")
   ...
   ans=backTrack(assignments,Domain,inpHoris,inpVerts,nRows,nCols,False)    # change False to True to run with MAC 
   # print (ans)
